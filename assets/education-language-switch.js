(() => {
  ...
  const links = () => `
    <a href="/education/"${current === 'zh-cn' ? ' aria-current="page"' : ''}>简中</a>
  ...
  if (footer && !footer.querySelector('.footer-language-links')) {
    ...
    wrap.setAttribute('aria-label', '语言');
    wrap.innerHTML = `<a href="/education/"${current === 'zh-cn' ? ' aria-current="page"' : ''}>简中</a><i>·</i><a href="/education/zh-tw/"${current === 'zh-tw' ? ' aria-current="page"' : ''}>繁中</a>`;
