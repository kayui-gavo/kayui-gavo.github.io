(() => {
  ...
  if (list) {
    const row = (meta, institution, position, responsibilities, extraClass = '') => `
      <article class="exp-row ${extraClass}">
        ...
        <ul class="exp-responsibilities">${responsibilities.map(item => `<li>${item}</li>`).join('')}</ul>
      </article>`;
  ...
  if (feedback) {
    ...
    const sources = [
      '/assets/student-feedback-wechat-v5.webp?v=20260814g',
      '/assets/student-feedback-wechat.webp?v=20260814g',
      '/assets/student-record-wall-v4.webp?v=20260814g'
    ];
    let i = 0;
